...
export default function ProductTable() {
  ...
  const handleEditSubmit = (values: any) => {
    if (editProduct && editProduct._id) {
        // CORRECTED: Ensure the ID is a string
        const productIdAsString = String(editProduct._id);
        dispatch(updateProduct({ productId: productIdAsString, product: values }));
        setEditDialogOpen(false);
        setEditProduct(null);
    }
  };
          ...
          <TableBody>
            {sellerProduct.products.map((item) => (
              <StyledTableRow key={item._id}>
                ...
                <StyledTableCell align="right">
                  <IconButton color='error' onClick={() => item._id && handleDeleteClick(String(item._id))}>
                    <DeleteIcon />
                  </IconButton>
                </StyledTableCell>
              </StyledTableRow>
